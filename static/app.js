...
async function loadAll(withToast = false) {
  await Promise.all([loadMatches(false), loadRanking(false)]);
...
function renderMatches() {
  ...
  if (!matches.length) {
    list.innerHTML = `<div class="empty-state">${state.scope === "upcoming" ? "Nenhuma partida nas próximas 24 horas." : "Nenhuma partida encontrada."}</div>`;
...
function filteredMatches() {
  ...
  const sortMode = state.scope === "upcoming" ? "time" : state.filters.sort;
  matches.sort((a, b) => {
    if (sortMode === "group") {
